...
// --- Global Share Modal ---
window.showShareModal = function(url, title) {
    let overlay = document.getElementById('share-modal-overlay');
    
    if (!overlay) {
        const html = `
            <div class="modal-overlay" id="share-modal-overlay">
                <div class="upload-modal" style="max-width: 400px;">
                    <div class="modal-header">
                        <h2>Share Document</h2>
                        <button type="button" class="close-modal-btn" id="close-share-modal"><i class="fas fa-times"></i></button>
                    </div>
                    <div style="padding: 24px;">
                        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin-bottom: 25px; text-align: center;">
                            <a href="#" id="share-wa" target="_blank" style="color: #25D366; font-size: 2.2rem; text-decoration: none; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
                                <i class="fab fa-whatsapp"></i>
                                <div style="font-size: 0.8rem; color: var(--text-dark); margin-top: 8px; font-weight: 500;">WhatsApp</div>
                            </a>
                            <a href="#" id="share-tg" target="_blank" style="color: #0088cc; font-size: 2.2rem; text-decoration: none; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
                                <i class="fab fa-telegram-plane"></i>
                                <div style="font-size: 0.8rem; color: var(--text-dark); margin-top: 8px; font-weight: 500;">Telegram</div>
                            </a>
                            <a href="#" id="share-in" target="_blank" style="color: #0A66C2; font-size: 2.2rem; text-decoration: none; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
                                <i class="fab fa-linkedin-in"></i>
                                <div style="font-size: 0.8rem; color: var(--text-dark); margin-top: 8px; font-weight: 500;">LinkedIn</div>
                            </a>
                            <a href="#" id="share-email" target="_blank" style="color: #EA4335; font-size: 2.2rem; text-decoration: none; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
                                <i class="fas fa-envelope"></i>
                                <div style="font-size: 0.8rem; color: var(--text-dark); margin-top: 8px; font-weight: 500;">Email</div>
                            </a>
                        </div>
                        <div class="form-group" style="margin-bottom: 0;">
                            <label class="form-label" style="font-size: 0.9rem;">Or copy link</label>
                            <div style="display: flex; gap: 10px;">
                                <input type="text" id="share-url-input" class="form-input" readonly value="">
                                <button type="button" id="share-copy-btn" class="btn btn-blue" style="border-radius: 8px; padding: 10px 16px; flex-shrink: 0;"><i class="fas fa-copy"></i></button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    document.getElementById('share-wa').href = `https://api.whatsapp.com/send?text=${encodeURIComponent(title + '\n\n' + url)}`;
    ...
    document.getElementById('share-email').href = `mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent('Check out this document:\n\n' + url)}`;
    ...
    copyBtn.onclick = () => {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(url).then(successFeedback);
        } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const buildMaterialCardHTML = (mat) => {
        ...
        return `
            <a href="/viewer.html?note=${mat.slug || safeId}" target="_blank" class="document-card" data-pdf-url="${mat.fileUrl}">
    ...
    // Theme Toggle Logic
    const themeBtns = document.querySelectorAll('#theme-toggle, #theme-toggle-nav, #theme-toggle-mobile, #theme-toggle-viewer');
    ...
    const isViewerPage = page === 'viewer.html' || window.location.pathname.includes('/notes/');
    if (isViewerPage) {
        ...
        // Override hardware/swipe back button to redirect to index.html
        ...
        let lastScrollY = 0;
        const topBar = document.querySelector('.viewer-top-bar');
        const pdfRenderContainerForScroll = document.getElementById('pdf-render-container');

        if (topBar && pdfRenderContainerForScroll) {
            pdfRenderContainerForScroll.addEventListener('scroll', () => {
                const currentScrollY = pdfRenderContainerForScroll.scrollTop;
        ...
        let fileTitle = urlParams.get('title');
        let fileDocId = urlParams.get('doc');
        let currentViewerMaterial = null;

        const pathParts = window.location.pathname.split('/');
        const slugIndex = pathParts.indexOf('notes');
        let urlSlug = slugIndex !== -1 && pathParts.length > slugIndex + 1 ? pathParts[slugIndex + 1] : null;

        if (!urlSlug) {
            urlSlug = urlParams.get('note');
        }

        if (urlSlug) {
            fileUrl = ''; // Defer PDF loading until fetch via slug completes
        } else if (!fileUrl) {
            fileUrl = 'https://firebasestorage.googleapis.com/v0/b/studybook-15297.firebasestorage.app/o/uploads%2Fstudy-materials%2F1774148361381_Engineering_Physics_2024-25.pdf?alt=media&token=0db344ad-b233-4c61-9bad-c84a8fd95ae2';
        }
        ...
        if (typeof pdfjsLib !== 'undefined') {
            // Toggle fullscreen on double click for a better user experience
            document.addEventListener('dblclick', () => {
                const viewerContainer = document.getElementById('pdf-viewer-container');
                if (!document.fullscreenElement) {
                    if (viewerContainer) {
                        viewerContainer.requestFullscreen().catch(err => {
                            console.error(`Error attempting to enable fullscreen on double-click: ${err.message}`);
                        });
                    }
                } else {
            ...
            if (container) {
                const renderPage = (num, canvas) => {
                    pdfDoc.getPage(num).then((page) => {
                        ...
                        // Calculate scale to fit 100% of the container width
                        const containerWidth = container.clientWidth || window.innerWidth;
                        const padding = window.innerWidth <= 768 ? 0 : 40;
                        const fitScale = (containerWidth - padding) / unscaledViewport.width;
                        const renderScale = fitScale * scale;
                        
                        const viewport = page.getViewport({scale: renderScale});
                        
                        // High DPI rendering to prevent blur
                ...
                container.addEventListener('touchend', (e) => {
                    if (pinchStartDist > 0 && e.touches.length < 2) {
                        pinchStartDist = 0;
                        scale = Math.max(0.5, Math.min(3, scale * tempScale));
                        tempScale = 1;
                ...
                window.loadViewerPDF = (url) => {
                    if (!url) return;
                    pdfjsLib.getDocument(url).promise.then((pdfDoc_) => {
                        pdfDoc = pdfDoc_;
                        const pageCountDisplay = document.getElementById('v-doc-pages');
                        if (pageCountDisplay) pageCountDisplay.textContent = `${pdfDoc.numPages} Pages`;
                        setupProgressiveRendering();
                    }).catch(error => {
                        console.error("Error loading PDF:", error);
                        const pageCountDisplay = document.getElementById('v-doc-pages');
                        if (pageCountDisplay) pageCountDisplay.textContent = 'Error loading PDF';
                    });
                };
                
                if (fileUrl) {
                    window.loadViewerPDF(fileUrl);
                }
            }
        }
        ...
        const loadDocumentMetadata = async () => {
            if (!window.getMaterialByUrl || (urlSlug && !window.getMaterialBySlug)) {
                setTimeout(loadDocumentMetadata, 500);
                return;
            }
            
            let mat = currentViewerMaterial;
            if (urlSlug && !mat) {
                mat = await window.getMaterialBySlug(urlSlug);
                if (mat) {
                    fileUrl = mat.fileUrl;
                    fileTitle = mat.title;
                    fileDocId = mat.id;
                    ...
                    if (saveBtn) saveBtn.dataset.docId = mat.id;
                    const docTitle = document.querySelector('.v-doc-title');
                    if (docTitle) { docTitle.textContent = mat.title; docTitle.title = mat.title; }
                    
                    if (window.loadViewerPDF) {
                        window.loadViewerPDF(mat.fileUrl);
                    }
                }
            } else if (!mat && fileUrl) {
                mat = await window.getMaterialByUrl(fileUrl);
            }

            if (mat) {
                currentViewerMaterial = mat;
                
                // Dynamic OG Tags & Title Update
                document.title = `${mat.title} - Study Book`;
                document.querySelector('meta[property="og:title"]')?.setAttribute("content", `${mat.title} - Study Book`);
                document.querySelector('meta[property="og:description"]')?.setAttribute("content", mat.description || `${mat.subject || mat.course} notes uploaded by a student.`);
        ...
        if (infoPanel) {
            ...
            infoPanel.addEventListener('touchend', (e) => {
                ...
                const infoBody = infoPanel.querySelector('.v-info-body');
                
                // Close if swiped down by > 80px, on mobile, and scroll is at the top
                if (deltaY > 80 && window.innerWidth <= 768 && infoPanel.classList.contains('active')) {
                    if (!infoBody || infoBody.scrollTop <= 10) {
                        togglePanel();
                    }
                }
            }, { passive: true });
        ...
        if (shareBtn) {
            shareBtn.addEventListener('click', async () => {
                ...
                // Use native share to open system share sheet for multiple apps
                if (navigator.share) {
                    try {
                        ...
                        if (error.name !== 'AbortError') window.showShareModal(urlToShare, shareTitle);
                    }
                } else {
                    window.showShareModal(urlToShare, shareTitle);
                }
            });
        }
    }
    ...
    if (page === 'search.html') {
        ...
        const renderCards = (filtered) => {
            ...
            filtered.forEach((mat, index) => {
                ...
                const cardHTML = `
                    <a href="/viewer.html?note=${mat.slug || mat.id}" target="_blank" class="document-card" data-pdf-url="${mat.fileUrl}">
                        <div class="doc-preview-container">
                            <canvas class="pdf-preview-canvas" style="display: none;"></canvas>
                            <div class="preview-loader"><i class="fas fa-file-pdf"></i><span>Preparing preview...</span></div>
                        </div>
            ...
            const loadRelatedNotes = async () => {
                ...
                try {
                    ...
                    filtered.forEach((mat) => {
                        ...
                        let iconColor = (mat.type && mat.type.includes('PYQ')) ? 'color: var(--purple-accent);' : '';
                        const cardHTML = `
                            <a href="/viewer.html?note=${mat.slug || mat.id}" class="document-card" data-pdf-url="${mat.fileUrl}">
                                <div class="doc-preview-container">
                                    <canvas class="pdf-preview-canvas" style="display: none;"></canvas>
                                    <div class="preview-loader"><i class="fas fa-file-pdf"></i><span>Preparing preview...</span></div>
                                </div>
    ...
    const loadRecentMaterials = async () => {
        ...
        if (continueReadingGrid) {
            ...
            if (getContinueReadingList().length === 0) {
        ...
        if (!window.getPersonalizedMaterials) {
        ...
        try {
            ...
            if (uid) {
                // Fetch materials personalized to the user's profile. This function now includes fallbacks.
                materials = await window.getPersonalizedMaterials(uid, 20);
            } else {
                // If user is not logged in, show the most recent materials.
                materials = await window.getRecentMaterials(20);
            }
            ...
            const emptyMessage = 'No study materials have been uploaded yet. Be the first to share!';
            ...
            if (recentlyUploadedGrid) {
                if (recentlyUploaded.length === 0) {
                    recentlyUploadedGrid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: var(--text-light); font-weight: 500; padding: 20px;">${emptyMessage}</p>`;
                } else {
            ...
            if (notesGrid) {
                if (notes.length === 0) {
                    notesGrid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: var(--text-light); font-weight: 500; padding: 20px;">${emptyMessage}</p>`;
                } else {
            ...
            if (pyqsGrid) {
                if (pyqs.length === 0) {
                    pyqsGrid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: var(--text-light); font-weight: 500; padding: 20px;">${emptyMessage}</p>`;
                } else {
...
>>>>>>> 1af2d087193d90a7d6622d153ab335f57fc16331
